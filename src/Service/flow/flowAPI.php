...
$key: '%kernel.project_dir%/keys/comercio.pem'
...
*/
  ...
  private $logPath; private $comercio; private $urlexito; private $urlconfirmacion; private $urlpago; private $key;
  private $tasa_default;
  private $tipo_integracion;
  private $url_fracaso;
  ...
  function __construct($log_path, $comercio, $url_exito, $url_confirmacion, $url_fracaso, $url_pago, $key, $tasa_default, $tipo_integracion) {
    $this->order["OrdenNumero"] = "";
    $this->order["Concepto"]    = "";
    $this->order["Monto"]       = "";
    $this->order["Comision"]    = $tasa_default;
    ...
    $this->urlexito = $url_exito;
    $this->urlconfirmacion = $url_confirmacion;
    $this->urlpago = $url_pago;
    $this->key = $key;
    $this->tasa_default = $tasa_default;
    $this->tipo_integracion = $tipo_integracion;
    $this->url_fracaso = $url_fracaso;
  }
  ...
  */
  public function new_order($orden_compra, $monto,  $concepto, $email_pagador, $tipo_comision = "Non") {
    ...
    if($tipo_comision == "Non") {
      ...
      //$tipo_comision = $container->getParameter('mailer.transport');
    }
    if(!is_numeric($monto)) {
  ...
  */
  public function flow_log($message, $type) {
    ...
    try{
      $file = @fopen($logPath . "/flowLog_" . date("Y-m-d") .".txt" , "a+");
  ...
  private function flow_get_private_key_id() {
    ...
    if (!$this->getKeyDirectory()) {
      throw new Exception("Clave privada de Comercio Flow no definida.");
    }

    if (!is_file($this->getKeyDirectory())) {
      throw new Exception($this->getKeyDirectory());
    }

    try {
      $fp = fopen($this->getKeyDirectory(), "r");
      $priv_key = fread($fp, 8192);
      fclose($fp);
      return openssl_get_privatekey($priv_key);
    } catch (Exception $e) {
      $this->flow_log("Error al intentar obtener la llave privada - Error-> " .$e->getMessage(), $this->getKeyDirectory());
      throw new Exception($e->getMessage());
    }
  }
  ...
  private function flow_pack() {
    $comercio = urlencode('joscri2698@gmail.com');
    ...
    $url_exito = urlencode('www.google.com');
    $url_fracaso = urlencode('www.google.com');
    $url_confirmacion = urlencode('www.google.com');
    $tipo_integracion = urlencode('f');
